...
<body>
	<%
		...
		List<StudentProfessor> studentprofessor = service.RegisterHistory();
		request.setAttribute("studentprofessor", studentprofessor);
		%>
	<h1 style="color: #FF69B4; text-align: center">Register History</h1>
	<TABLE border="1" style="margin: 0 auto; text-align: center; border: 5px">
		...
		<c:forEach var="i" items="${studentprofessor}">
			<TR style="color: #696969">
				<td>${i.noStudent}</td>
				<td>${i.nameStudent}</td>
				<td>${i.noProfessor}</td>
				<td>${i.nameProfessor}</td>
			</TR>
		</c:forEach>
	</TABLE>
